/**
 * K6 Load Test - Stress Test (500 concurrent users)
...
export default function () {
  ...
  const signupPageCheck = check(signupPageResponse, {
    ...
    'signup page response time < 5s': (r) => r.timings.duration < 5000,
  });
  ...
  sleep(0.5); // Shorter wait for stress test
  ...
  const signupCheck = check(signupResponse, {
    ...
    'signup response time < 10s': (r) => r.timings.duration < 10000,
  ...
  if (signupResponse.status === 302 || signupResponse.status === 200) {
    ...
    const successCheck = check(successResponse, {
      ...
      'success page response time < 5s': (r) => r.timings.duration < 5000,
    });
  ...
  sleep(0.5); // Final wait
}

export function handleSummary(data) {
  return {
    'reports/stress_test_results.json': JSON.stringify(data, null, 2),
    'reports/stress_test_summary.html': generateHTMLReport(data, 'Stress Test Results'),
  };
}
  ...
  return `
    ...
    <body>
        ...
        <div class="metrics">
            ...
            <div class="metric">
                <strong>Error Rate:</strong> <span class="${errorRate > 10 ? 'error' : errorRate > 5 ? 'warning' : 'good'}">${errorRate}%</span>
            </div>
            <div class="metric">
        ...
        <div class="analysis">
            <h2>Analysis</h2>
            <p><strong>Test Type:</strong> Stress Test</p>
            <p><strong>Peak Concurrent Users:</strong> 500</p>
            <p><strong>Duration:</strong> 18 minutes (2m ramp up, 10m steady, 2m ramp down)</p>
            <p><strong>Target:</strong> Assess system behavior under high load and identify breaking point</p>
            
            <h3>Key Findings:</h3>
            <ul>
                <li>System performance under stress conditions</li>
                <li>Response time degradation patterns</li>
                <li>Error rate increase under load</li>
                <li>Resource utilization bottlenecks</li>
            </ul>
        </div>
        ...
        <div class="recommendations">
            ...
            <ul>
                ...
                <li>Consider auto-scaling solutions</li>
            </ul>
        </div>
    </body>
    </html>
