/**
 * K6 Load Test - Spike Test (Sudden jump to 1000 users)
...
export const options = {
  stages: [
    ...
    { duration: '30s', target: 1000 }, // Spike: sudden jump to 1000 users
...
export default function () {
  ...
  const uniqueEmail = user.email.replace('@', `+${timestamp}${randomSuffix}@`);
  ...
  const signupPageCheck = check(signupPageResponse, {
    ...
    'signup page response time < 15s': (r) => r.timings.duration < 15000,
  });
  ...
  sleep(0.2); // Very short wait for spike test
  ...
  const signupCheck = check(signupResponse, {
    ...
    'signup response time < 20s': (r) => r.timings.duration < 20000,
  ...
  if (signupResponse.status === 302 || signupResponse.status === 200) {
    ...
    const successCheck = check(successResponse, {
      ...
      'success page response time < 10s': (r) => r.timings.duration < 10000,
    });
  ...
  sleep(0.3); // Final wait
}

export function handleSummary(data) {
  return {
    'reports/spike_test_results.json': JSON.stringify(data, null, 2),
    'reports/spike_test_summary.html': generateHTMLReport(data, 'Spike Test Results'),
  };
}
  ...
  return `
    ...
    <head>
        ...
        <style>
            ...
            .error { color: red; }
            .chart { margin: 20px 0; padding: 20px; background-color: #f0f0f0; border-radius: 5px; }
            .spike-info { background-color: #fff3cd; padding: 15px; border-radius: 5px; margin: 20px 0; }
        </style>
    </head>
    <body>
        <div class="header">
        ...
        <div class="spike-info">
            <h3>⚠️ Spike Test Characteristics</h3>
            <p>This test simulates a sudden traffic spike from 10 to 1000 concurrent users in 30 seconds, 
            which is a realistic scenario for viral content or marketing campaigns.</p>
        </div>
        ...
        <div class="metrics">
            ...
            <div class="metric">
                <strong>Error Rate:</strong> <span class="${errorRate > 20 ? 'error' : errorRate > 10 ? 'warning' : 'good'}">${errorRate}%</span>
            </div>
            <div class="metric">
        ...
        <div class="analysis">
            <h2>Analysis</h2>
            <p><strong>Test Type:</strong> Spike Test</p>
            <p><strong>Peak Concurrent Users:</strong> 1000</p>
            <p><strong>Duration:</strong> 5.5 minutes (1m normal, 30s spike, 2m peak, 1m normal, 1m ramp down)</p>
            <p><strong>Target:</strong> Assess system behavior under sudden traffic spikes</p>
            
            <h3>Key Findings:</h3>
            <ul>
                <li>System recovery time after spike</li>
                <li>Performance degradation during spike</li>
                <li>Error rate spike during peak load</li>
                <li>System stability after traffic returns to normal</li>
            </ul>
        </div>
        ...
        <div class="recommendations">
            ...
            <ul>
                ...
                <li>Consider queue-based processing for high-volume operations</li>
                <li>Monitor system metrics during traffic spikes</li>
        ...
        <div class="chart">
            ...
            <p>Normal Load (10 users) → Spike (1000 users) → Peak (1000 users) → Normal (10 users)</p>
            <p>This pattern tests the system's ability to handle sudden increases in traffic and recover gracefully.</p>
        </div>
    </body>
    </html>
  `;
